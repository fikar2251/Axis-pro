...
<head>
    @php
        ...
        header('Content-Disposition: attachment; filename=Laporan Case List.xls');
    @endphp
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Laporan Case Lists Tanggal {{ Carbon\Carbon::parse($from)->format('d/m/Y') }} s.d
        {{ Carbon\Carbon::now()->format('d/m/Y') }} - {{ $status == 5 ? 'Closed File' : $status }}</title>
                            ...
                            @foreach ($case as $caselist)
                                ...
                                @foreach ($caselist->member as $member)
                                    <tr>
                                        ...
                                        <td>{{ Carbon\Carbon::parse($caselist->instruction_date)->format('d/m/Y') }}
                                        </td>
                                        <td>{{ Carbon\Carbon::parse($caselist->survey_date)->format('d/m/Y') }}</td>
                                        <td>{{ $caselist->now_update != null ? Carbon\Carbon::parse($caselist->now_update)->format('d/m/Y') : $caselist->now_update }}
                                        </td>
                                        <!-- All & Outstanding -->
                                        @if ($status != '5')
                                            <td>{{ $caselist->ia_date != null ? Carbon\Carbon::parse($caselist->ia_date)->format('d/m/Y') : $caselist->ia_date }}
                                            </td>
                                            <td>{{ $caselist->ia_curr }}</td>
                                            <td>{{ number_format($caselist->ia_amount, 2) }}</td>
                                            <td>{{ $caselist->pr_date != null ? Carbon\Carbon::parse($caselist->pr_date)->format('d/m/Y') : $caselist->pr_date }}
                                            </td>
                                            <td>{{ $caselist->pr_curr }}</td>
                                            <td>{{ number_format($caselist->pr_amount, 2) }}</td>
                                            <td>{{ $caselist->ir_st_date != null ? Carbon\Carbon::parse($caselist->ir_st_date)->format('d/m/Y') : $caselist->ir_st_date }}
                                            </td>
                                            <td>{{ $caselist->ir_st_curr }}</td>
                                            <td>{{ number_format($caselist->ir_st_amount, 2) }}</td>
                                            <td>{{ $caselist->ir_nd_date != null ? Carbon\Carbon::parse($caselist->ir_nd_date)->format('d/m/Y') : $caselist->ir_nd_date }}
                                            </td>
                                            <td>{{ $caselist->ir_nd_curr }}</td>
                                            <td>{{ number_format($caselist->ir_nd_amount, 2) }}</td>
                                            <td>{{ $caselist->pa_date != null ? Carbon\Carbon::parse($caselist->pa_date)->format('d/m/Y') : $caselist->pa_date }}
                                            </td>
                                            <td>{{ $caselist->pa_curr }}</td>
                                            <td>{{ number_format($caselist->pa_amount, 2) }}</td>
                                            <td>{{ $caselist->fr_date != null ? Carbon\Carbon::parse($caselist->fr_date)->format('d/m/Y') : $caselist->fr_date }}
                                            </td>
                                        ...
                                        <td>{{ $caselist->date_instruction != null ? Carbon\Carbon::parse($caselist->date_insctruction)->format('d/m/Y') : $caselist->date_instruction }}
                                        </td>
                                        ...
                                        @endphp
                                        <td>{{ $caselist->currency == 'IDR' ? number_format($caselist->claim_estimate, 2) : number_format(0, 2) }}
                                        </td>
                                        <td>{{ $caselist->currency == 'USD' ? number_format($caselist->claim_estimate, 2) : number_format(0, 2) }}
                                        </td>
                                        @if ($status == 'all' || $status == '5')
                                            <td>{{ $caselist->currency == 'IDR' ? number_format($caselist->claim_amount, 2) : number_format(0, 2) }}
                                            </td>
                                            <td>{{ $caselist->currency == 'USD' ? number_format($caselist->claim_amount, 2) : number_format(0, 2) }}
                                            </td>
                                            <td>{{ $caselist->claim_amount_curr == 'IDR' ? number_format(($caselist->fee_idr * $member->share) / 100, 2) : number_format(0, 2) }}
                                            </td>
                                            <td>{{ $caselist->claim_amount_curr == 'USD' ? number_format(($caselist->fee_usd * $member->share) / 100, 2) : number_format(0, 2) }}
                                            </td>
                                            <td>{{ $caselist->claim_amount_curr == 'IDR' ? number_format(($caselist->expense()->sum('total') * $member->share) / 100, 2) : number_format(0, 2) }}
                                            </td>
                                            <td>{{ $caselist->claim_amount_curr == 'USD' ? number_format(($caselist->expense()->sum('total') * $member->share) / 100, 2) : number_format(0, 2) }}
                                            </td>
                                            <td>
                                                @php
