...
class CaseListController extends Controller
{
            ...
            return datatables()->of($data)
                ...
                ->editColumn('is_leader', function ($row) {
                    ...
                    foreach ($row->member as $data) {
                        if ($row->remark == NULL) {
                            $html .= '<li>' . $data->caselist->brand . '-' . '(' . $data->share . ')' . '-' . '<strong>' . $data->is_leader  . '</strong>' . '<li>';
                        } else {
                            $html .= '<li class="text-danger">' . $data->caselist->brand . '-' . '(' . $data->share . ')' . '-' . '<strong>' . $data->is_leader  . '</strong>' . '<li>';
                        }
                    };
                    $html .= '
                        </ul>
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            return redirect()
                    ->back()
                    ->with('last_focus', $request->last_focus)
